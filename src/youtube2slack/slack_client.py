...
class SlackClient:
    ...
    def send_image(self, image_path: str, title: str = "Image", 
                   comment: Optional[str] = None, retry_on_rate_limit: bool = True) -> bool:
        """Send image to Slack as attachment.
        ...
        try:
            ...
            # For webhook URLs, we can't directly upload files
            # Instead, we'll use the fallback approach with base64 encoding
            import base64
            
            # Encode image to base64
            image_b64 = base64.b64encode(image_data).decode('utf-8')
            
            # Create a message with image reference
            message_text = f"🖼️ {title}"
            if comment:
                message_text += f"\n{comment}"
            
            # Send as a message with image data reference
            payload = {
                "text": message_text,
                "attachments": [
                    {
            ...
            # Note: Base64 data URLs in Slack have size limitations
            # For production use, consider uploading to external service
            if len(image_b64) > 50000:  # ~50KB limit for inline images
                logger.warning("Image too large for inline display, sending as reference")
                return self.send_message(f"🖼️ {title}\n{comment or 'Scene change detected'}")
            ...
            fallback_msg = f"🖼️ {title}\n{comment or 'Scene change detected'}\n(Image upload failed)"
            return self.send_message(fallback_msg, retry_on_rate_limit)